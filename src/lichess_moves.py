""" All the possible UCI moves in the lichess dataset. """
...
def convert_fen_to_fixed_length(fen: str) -> str:
    ...
    EXPECTED_SEQ_LENGTH = 81
    assert len(fixed_fen) == EXPECTED_SEQ_LENGTH, f"Expected {EXPECTED_SEQ_LENGTH} characters, got {len(fixed_fen)}"
    return fixed_fen
...
if __name__ == '__main__':
  ...
  for i in range(len(val_r)):
    fen, move = encode_message(val_r[i], fixed_fen=False)
    ALL_MOVES.add(move)

  print("ALL MOVES", len(ALL_MOVES))
  import tqdm
  for i in tqdm.tqdm(range(5_000_000)):
    fen, move = encode_message(train_r[i], fixed_fen=False)
    ALL_MOVES.add(move)

#   for i in tqdm.tqdm(range(len(train_r))):
#     fen, move = encode_message(train_r[i], fixed_fen=False)
#     ALL_MOVES.add(move)
  ...
  print(ALL_MOVES)
  import pickle 
  pickle.dump(ALL_MOVES, open('all_moves_medium.pkl', 'wb'))
