...
if __name__ == '__main__':
  ...
  for dataset_name, max_records in dataset_sizes.items():
    ...
    with open(f'./data/{dataset_name}/train.jsonl', 'w') as f:
      for i in range(0, max_records):
        fen, move = encode_message(train_r[i], fixed_fen=FIXED_FEN)
        json.dump({
          'prompt': fen,
          'response': move,
        }, f)
        f.write('\n')
    with open(f'./data/{dataset_name}/val.jsonl', 'w') as f:
      for i in range(0, len(val_r)):
